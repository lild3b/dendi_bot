...
async def pnl(interaction: Interaction, user: discord.Member = None):
...
async def pnl_summary(interaction: Interaction, user: discord.Member = None):
    target = user or interaction.user
    user_data = get_user_pnl(str(target.id))
...
async def summarize(interaction: Interaction, limit: int = 20):
    try:
...
async def on_app_command_error(
    interaction: Interaction, error: discord.app_commands.AppCommandError
):
